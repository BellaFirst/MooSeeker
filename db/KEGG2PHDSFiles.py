...
class KEGG(object):
    ...
    def _download_reactions(self):
        ...
        for reaction in reactions:
            ...
            data = name_equation.split('; ') #['polyphosphate polyphosphohydrolase', 'Polyphosphate + n H2O <=> (n+1) Oligophosphate']
            equation = data[-1]
            if len(data) == 2:
                name = data[0]
            elif len(data) == 1:
            ...
            self._rxn[rxn_code] = self.initialize_reaction() #{'R00001': {'name': [], 'identifiers': {'KEGG': []}, 'enzymes': [], 'equation': '', 'substrates': [], 'products': [], 'Scoef': [], 'Pcoef': [], 'reversibility': 0, 'use': True}}
            ...
            for R in KEYS:
                ...
                if (self._organism['reactions'][R]['use'] == True):
                    ...
                    if reversibility == 0:
                        ...
                        for coef, S in zip(self._organism['reactions'][R]['Scoef'],self._organism['reactions'][R]['substrates']): # 把具体的化合物名字改成C
...
parser.add_argument('-r', '--use_reversibility_info', default='False', type=(lambda x: x.lower() in ("yes", "true", "t", "1")),
