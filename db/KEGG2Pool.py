...
# from lib.SingleNode import SingleReaction
from dGPredictor.utils import get_dG0
from dGPredictor.utils import load_model
from dGPredictor.utils import load_molsig_rad1
from dGPredictor.utils import load_molsig_rad2
from dGPredictor.utils import load_smiles
from dGPredictor.utils import parse_formula
...
class KEGG(object):
    ...
    def __init__(self, usedict=None):
        ...
        self._pool = dict()  # mypool
        self._rxn = dict()  # reaction
        self._cpd = dict()  # compounds
        self._gly = dict()  # glycans

        self.folder_name = './KEGG_20220416'
        if not os.path.exists(self.folder_name):
            os.makedirs(self.folder_name)

        self.destination_folder = self.folder_name
    ...
    def _download_relation_glycan_cpd(self):
        ...
        for gl2cpd in GL2CPD:
            # 'gl:G11113\tcpd:C00008'

            gl, cpd = re.findall('gl:(G\d{5})\tcpd:(C\d{5})', gl2cpd)[0]  # 正则化，查找匹配项
            # gl:'G11113', cpd:'C00008'
        ...
        for gl2name in GL2NAME:
            # 'gl:G00001\tN-Acetyl-D-glucosaminyldiphosphodolichol; (GlcNAc)1 (PP-Dol)1'

            gl, names = gl2name.replace('gl:', '').split('\t')
            # gl: 'G00001'
            # names: 'N-Acetyl-D-glucosaminyldiphosphodolichol; (GlcNAc)1 (PP-Dol)1'
    ...
    def _download_reactions(self):
        ...
        # =======================================================
        # CORRECCION DE ERRORES EN LA BASE DE DATOS 更正数据库中的错误
        # ===============================================
        ...
        reactions = reactions.replace('rn:', '').strip()  # .strip()表示删除空格
        ...
        for reaction in reactions:
            ...
            self._rxn[
                rxn_code] = self.initialize_reaction()  # {'R00001': {'name': [], 'identifiers': {'KEGG': []}, 'enzymes': [], 'equation': '', 'substrates': [], 'products': [], 'Scoef': [], 'Pcoef': [], 'reversibility': 0, 'use': True}}
            ...
            for s in S[:]:
                ...
                coef = re.findall(
                    '^\d+\ |^[1-9nm]+\ |^\([1-9nm]+\)\ |\([1-9nmx]+\)$|^\([nm][+-][nm\d]\)\ |^[nm][+-][nm1-3]\ |\([nm][+-][nmx\d]\)$',
                    s)
    ...
    def _download_enzymes(self):
        ...
        for enzyme in enzymes:
            # re.findall 返回string中左右想匹配的子串，返回形式为数组
    ...
    def download_organism_data(self, organism='rn', pathways='all', use_reversibility_info=False):
        '''
        use_reversibility_info=True --> Download information of reversibility 下载可逆信息
        '''
        ...
        for R in self._rxn.keys():
            ...
            # -----------------------------------
            if ((organism != 'rn') and (R not in self._organism['reactions'].keys())) or (
                    (organism == 'rn') and (pathways != 'all')):
                ...
                for enzyme in self._rxn[R]['enzymes']:
                    ...
                    if insert:  # INSERTO TODAS COMO NO USADAS INICIALMENTE 插入所有最初未使用的内容
        ...
        if pathways == 'all':
            ...
            pathways = self._query.request('GET', 'http://rest.kegg.jp/list/pathway/%s' % organism)  # 此代码2
            ...
            _pathways = [[_pathway for _pathway in _pathways if pathway_code in _pathway][0] for pathway_code in
                         pathways]

        # _pathways --> ['path:rn00010\tXXXXXXXX','path:rn...]

        # ============================= 这个 if 为 False
        if (organism != 'rn') or ((organism == 'rn') and (self._Npathways != 0)) or (
                (organism == 'rn') and (use_reversibility_info == True)):
            ...
            for REACTION in REACTIONS_org:
                ...
                reactions, rev = \
                re.findall('<reaction id=.\d*. name=.(rn:R\d{5}.*). type="(reversible|irreversible)">', REACTION)[0]
                            ...
                            if REVERSIBILITY.get(reaction, 0) != 0:
                                ...
                                if rev == 'reversible':
                                    ...
                                    if REVERSIBILITY[reaction]['reversibility'] != 'reversible':
                                        ...
                                        if (len(REVERSIBILITY[reaction]['S'].intersection(S)) == 0) and len(
                                                REVERSIBILITY[reaction]['P'].intersection(P)) == 0:
    ...
    def save_reactions_to_file(self, translate=True, organism=False):
        ...
        if (organism != False):
            ...
            filename_reactions = filename_reactions.replace('_rn_', '_' + self._organism['id'] + '-' + str(
                self._Npathways) + Npathways)
            ...
            for R in KEYS:
                ...
                if (self._organism['reactions'][R]['use'] == True):
                    ...
                    if (self.use_reversibility_info == True):  # Tengo en cuenta la reversibilidad 我考虑了可逆性
                    ...
                    if reversibility == 0:
                        ...
                        for coef, S in zip(self._organism['reactions'][R]['Scoef'],
                                           self._organism['reactions'][R]['substrates']):  # 把具体的化合物名字改成C

                            if translate:
                                S = self._translator[S]

                            REACTIONS += str(int(coef)) + ' ' + S + ' + '
                            trans_equation +=  str(int(coef)) + ' ' + S + ' + '

                        REACTIONS = REACTIONS[:-3] + ' <=> '
                        trans_equation = trans_equation[:-3] + ' <=> '

                        for coef, P in zip(self._organism['reactions'][R]['Pcoef'],
                                           self._organism['reactions'][R]['products']):

                            if translate:
                                P = self._translator[P]

                            REACTIONS += str(int(coef)) + ' ' + P + ' + '
                            trans_equation += str(int(coef)) + ' ' + P + ' + '
                    # ········································································
                    ...
                    if reversibility == 1:
                        ...
                        for coef, S in zip(self._organism['reactions'][R]['Scoef'],
                                           self._organism['reactions'][R]['substrates']):
                    ...
                    REACTIONS = REACTIONS[:-3] + '\n'
                    trans_equation = trans_equation[:-3]
                    self._rxn[R]['trans_equation'] = trans_equation
                    # ········································································
    ...
    # =================================================
    def save_rxn_dict(self):

        self.Get_dG()

        file_dir = self.destination_folder + '/RXN_DICT/'

        if not os.path.exists(file_dir): os.mkdir(file_dir)

        filename = file_dir + 'RXN_DICT_' + time.strftime("%Y%m%d") + '.json'

        print("===== Save RXN Dict =====")

        with open(filename, 'w') as f:

            json.dump(self._rxn, f)

        print("===== Done! =====")

    def save_my_pool(self, translate=True):

        file_dir = self.destination_folder + '/MYPOOL/'

        if not os.path.exists(file_dir): os.mkdir(file_dir)

        filename_reactions = file_dir + 'MYPOOL_' + time.strftime("%Y%m%d") + '.npy'
        ...
        for R in KEYS:
            ...
            if (self._rxn[R]['use'] == True):
                if (self.use_reversibility_info == True):
                    reversibility = self._rxn[R]['reversibility']
                else:
                    reversibility = 0
                ...
                if reversibility == 0:
                    ...
                    for S in self._rxn[R]['substrates']:

                        if translate: S = self._translator[S]

                        for P in self._rxn[R]['products']:

                            if translate: P = self._translator[P]

                            srn_front = SingleReaction(S, P, self._rxn[R],Tanimoto=self.Get_Tanimoto(S, P))

                            self._pool.setdefault(S, set()).add(srn_front)
                    ...
                    for P in self._rxn[R]['products']:
                        ...
                        for S in self._rxn[R]['substrates']:
                            ...
                            srn_back = SingleReaction(P, S, self._rxn[R], Tanimoto=self.Get_Tanimoto(P, S))

                            self._pool.setdefault(P, set()).add(srn_back)
        ...
        np.save(filename_reactions, self._pool)

        with open(file_dir + 'MYPOOL_' + time.strftime("%Y%m%d") + '.json', 'w') as f:
            
            json.dump(self._pool, f)

        print("===== Done! ======")

    def test_save_pool(self, translate=True):

        with open('/home/caoyh/project/myseeker/db/KEGG_20220411/RXN_DICT/RXN_DICT_20220414.json', 'r') as f:
        ...
        KEYS = list(myrxn.keys())

        for R in KEYS:

            if (myrxn[R]['use'] == True):
                reversibility = myrxn[R]['reversibility']
            ...
            #         reversibility = 0
                ...
                try:
                    if reversibility == 0:
                        ...
                        for S in myrxn[R]['substrates']:

                            if translate: S = self._translator[S]

                            for P in myrxn[R]['products']:

                                if translate: P = self._translator[P]

                                srn_front = SingleReaction(S, P, myrxn[R],Tanimoto=self.Get_Tanimoto(S, P))
                        ...
                        for P in myrxn[R]['products']:
                            ...
                            for S in myrxn[R]['substrates']:
                                ...
                                srn_back = SingleReaction(P, S, myrxn[R], Tanimoto=self.Get_Tanimoto(P, S))
    ...
    def save_MOLFile(self):

        print("===== Save Mol File =====")

        molfile_dir = self.destination_folder + '/KEGG_Compound_MOLiles/'

        if not os.path.exists(molfile_dir): os.mkdir(molfile_dir)

        CNAMES = list(self._cpd.keys())
    ...
    def Get_Tanimoto(self, s, p):
        ...
        if (not os.path.exists(molfile_dir + str(s) + ".mol")) or (not os.path.exists(molfile_dir + str(p) + ".mol")): return 0

        else:
            mols = []
            for cname in [str(s), str(p)]:
                mol = Chem.MolFromMolFile(molfile_dir + cname + ".mol")
                mols.append(mol)
            try:
    ...
    def Get_dG(self):
        ...
        #------------------------------------------------------------
        KEGG_rxn_dG_file = '/home/caoyh/project/myseeker/eQuilibrator/kegg_reactions_CC_ph70_quantity.tsv' 
        sbtabdoc = SBtab.read_csv(KEGG_rxn_dG_file, document_name='kegg_reactions_CC')
        sbtab = sbtabdoc.get_sbtab_by_id('t_01')
        data = sbtab.to_data_frame()
        ...
        for keys in self._rxn:   
            try:
                if keys in list(data['Reaction:Identifiers:kegg.reaction']) :
                    idx = data.index[(data['Reaction:Identifiers:kegg.reaction'] == keys)][0]
                    if data.loc[idx, 'Mean']!='' or data.loc[idx, 'Std']!='':  
                        mu, std = float(data.loc[idx, 'Mean']), float(data.loc[idx, 'Std'])
                    else:
                        kegg_rxn_string = self._rxn[keys]['trans_equation']
                        kegg_rxn_dict = parse_formula(kegg_rxn_string)
                        mu, std, _, _ = get_dG0(kegg_rxn_dict, keys, pH, I, loaded_model, molsig_r1, molsig_r2,[], [], [])
                else:
                    kegg_rxn_string = self._rxn[keys]['trans_equation']
                    kegg_rxn_dict = parse_formula(kegg_rxn_string)
                    mu, std, _, _ = get_dG0(kegg_rxn_dict, keys, pH, I, loaded_model, molsig_r1, molsig_r2,[], [], [])
                ...
                self._rxn[keys]['dG_Mean'], self._rxn[keys]['dG_Std'] = mu, std
                print(self._rxn[keys], self._rxn[keys]['dG_Mean'], self._rxn[keys]['dG_Std'])
...
parser = argparse.ArgumentParser(
    description='Build PhDSeeker files from KEGG rest service (requires internet connection).')
parser.add_argument('-o', '--organism', default='rn', help='Organism for which it is wanted to build the files.')
parser.add_argument('-p', '--pathways', default='all',
                    help='List of pathways to be downloaded. By default, all available pathways are used to build the dataset.')
parser.add_argument('-r', '--use_reversibility_info', default='False',
                    type=(lambda x: x.lower() in ("yes", "true", "t", "1")),
                    help='Boolean value indicating if reactions should be taken always as reversible.')
parser.add_argument('-d', '--usedict', default=None,
                    help='Specify a dictionary to translate KEGG codes into compound names.')
...
if __name__ == '__main__':
    ...
    # DATA.download_organism_data(organism=args['organism'], pathways=args['pathways'],
    #                             use_reversibility_info=args['use_reversibility_info'])
