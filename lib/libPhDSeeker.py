...
class Ant(object):
    ...
    def __init__(self, Rmax=100, idx=0, strict_initialization=False):
        ...
        self.idx = idx # self.ants = [Ant(Rmax=Rmax, idx=idx) for idx in range(0,Nants)]
        self.cost = np.inf
        self.found = False
...
#        P = np.array(P.todense()).astype(np.bool)
    ...
    def Initialize(self, pool, idx_initial, idx_produce, compounds, Rinitial):
        ...
        def 初始化（池、相关化合物、初始化合物、丰富的化合物、外部化合物、信息素、内存）

         此方法执行初始化蚂蚁所需的所有任务。
         - 清洁蚂蚁化合物组。
         - 建立不同的化合物组。
         - 为每个可能的起始化合物随机选择一个反应并将其插入。
         - 在插入每个反应后更新不同的化合物组。
        '''
...
#        self.Co = (self.Co).astype(np.bool)
    ...
    #===========================================
    def Search(self, S, P, FEROMONAS, Cnames, Rnames): #FEROMONAS 信息素矩阵
        """EXPLICACION"""
#        import timeit
        ...
        # ACTUALIZO COMPUESTOS DISPONIBLES 更新可用化合物
        self.Co[s] = True
        ...
        # INDICE DE LAS REACCIONES QUE AUN NO SON FACTIBLES 尚不可行的反应指数
        idx_infact = np.ones(Nr).astype(np.bool) # <--- Puedo determinar luego las factibles 然后我可以确定可行的
        
        # NUMERO DE SUSTRATOS QUE REQUIERE CADA REACCION 每个反应所需的底物数量
        NS = S.sum(axis=0)
        ...
        while not STOP:
            ...
            #------------------------------------
            # AGREGO PRODUCTOS 添加产品
            #-------------------------
            old_Co = self.Co.copy() # 可用于选择的化合物
            
            self.Co[p] = True # 把新路径中的产物加进去

            # 如果 待更新的产物在配置文件中涉及的化合物中，则 Co_updated = True
            if np.all(np.equal(old_Co, self.Co)): # np.all 与操作，判断矩阵中所有元素是否都是True
                Co_updated = False
            else:
                Co_updated = True
            #------------------------------------
            
            if (idx_infact.sum() > 0) and Co_updated: # idx_infact:(79,)
...
#            search = (binary_pathway[idx_infact == False] * feromonas[idx_infact == False]).cumsum()
            ...
            if (search.sum() == 0):
                ...
                r_selected = np.argmin((search/search.max()) < np.random.rand()) #在可选的候选中随机选择一个数 0-10 之间的一个数
                
                idx = np.argwhere(idx_infact == False).flatten()[r_selected] # 从可选的路径中再选择一条的索引
...
#        binary_compounds[[pool.Cnames.index(c) for c in compounds['relate']]] = 1
        ...
        # NON FORCED INITIAL COMPOUNDS 非强制初始化合物
    ...
    def Initialize(self, pool):
        ...
        # self.size 为初始化蚂蚁的个数 idxs = [1, 1, 1, 0, 1, 0, 1, 0, 0, 1]
        idxs = [np.argwhere(pRinitials >= np.random.rand()).min() for n in range(self.size)]

        Rinitials = [self.Rinitials[idx] for idx in idxs]# [15, 15, 15, 14, 15, 14, 15, 14, 14, 15]
...
class Pool(object):
    ...
    def __init__(self, RXNfile, ENZfile=None, COMPNAMEfile=None, DTYPE=np.float32):
        ...
        self._code2compname = dict() #所有的化合物的名字和化合物 eg:self._code2compname['C00001']=['H2O']
        self._enzymes = dict() #所有酶的名字和编号 eg:self._enzymes['R00001']=['3.6.1.10']
        
        self.Cnames = set() # reaction反应中涉及的所有化合物的名字
        self.Rnames = list() #reaction反应中涉及的所有路径的名字
        ...
        if ENZfile is not None:
            ...
            for enzyme in enzymes:
                R, ec_codes = enzyme.split(': ') #R: 酶的符号， ec_codes： 酶的编号
        ...
        if COMPNAMEfile is not None:
            ...
            for name in names: #name: 'C00001: H2O'
                code, translations = name.split(': ') #code='C00001', traslations='H2O'
        ...
        for reaction in reactions:
            ...
            # R:reaction名称，S底物化合物，P:产物化合物，
            # ‘coefficients’: 系数，'enzyme': 反应中涉及的酶,似乎都没有涉及到酶。。。
            # （此外，由于有的反应是双向的，所以这些反应会记两次）
        ...
        self.Cnames.sort()
        Nc = len(self.Cnames) #化合物的个数：60
        #==================================
        
        
        #==================================
        # REACTIONS
        Nr = len(SEMIREACTIONS) # 反应路径reaction的个数：79
        
        self.size = Nr
        #============
        
        self.S = np.zeros((Nc,Nr), dtype=np.bool) # 底物矩阵
        self.P = np.zeros((Nc,Nr), dtype=np.bool) # 产物矩阵
        self.M = lil_matrix((Nc,Nr), dtype=np.float32) #
        # lil_matrix使用两个列表保存非零元素。data保存每行中的非零元素，rows保存非零元素所在的列。
        # 这种格式也很适合逐个添加元素，并且能快速获取行相关的数据。适合增量构建，不适合批量构建。

        
        for idx_col in range(len(SEMIREACTIONS)):#len(SEMIREACTIONS)=79
